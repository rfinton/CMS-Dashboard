...
<body>
	...
	<div class="container" id="filter">
		...
		<p>Analysis of clinics offering a specified service.</p>
		...
		<form action="" class="row row-cols-lg-auto g-3 align-items-end">
			...
			<div style="width: 30%;">
				...
				<select name="service" id="service" class="form-select" style="width: 100%;" required>
	...
	<div id="results-view">
		<div id="clinics"></div>
		<div class="charts" style="display: initial; font-size: 30px;"></div>
		<div id="stats"></div>
	</div>
	...
	<script>
		let endpoint = '/cms/api/findserv/findserv.php';
		...
		document.forms[0].addEventListener('submit', function(ev) {
			...
			let service = encodeURIComponent(selected[3].value);
			let fq = encodeURIComponent(selected[4].value);
			let url_query_string = '?terr=' + terr;
			url_query_string += '&state=' + state;
			url_query_string += '&region=' + region;
			url_query_string += '&service=' + service;
